/**
 * minutes-diff-sum-all.js - 全量可转债分时差值计算工具
 *
 * 自动获取全部可转债代码，批量计算并输出结果，按红率降序排列。
 * 与 minutes-diff-sum.js 的区别：无需手动指定名称/代码，自动全量处理。
 *
 * 用法:
 *   node minutes-diff-sum-all.js --from 2026
 *   node minutes-diff-sum-all.js --from 20260101 --to 20260708
 *   node minutes-diff-sum-all.js --from 2026 --times "09:30-09:40" --limit 50
 *   node minutes-diff-sum-all.js --from 2026 --times "11:00-13:05" --mode pct --limit 30
 */
...
function printResult(results, showDetails, mode, limit) {
  ...
  const sep = "=".repeat(78);
  console.log(`\n${sep}`);
  console.log(`  全量可转债分时差值对比 (${timeLabel})`);
...
async function main() {
  ...
  const opts = { from: "", to: "", timeStart: "09:30", timeEnd: "09:40" };
  ...
  if (!opts.from) {
    console.log("用法: node minutes-diff-sum-all.js --from <YYYYMMDD|YYYY> [选项]");
    console.log("选项:");
    ...
    console.log("");
    console.log("计算规则: time_end价格 - time_start价格");
    console.log("示例:");
    console.log("  node minutes-diff-sum-all.js --from 2026");
    console.log("  node minutes-diff-sum-all.js --from 2026 --limit 30");
    console.log("  node minutes-diff-sum-all.js --from 2026 --times \"09:30-09:40\" --limit 50");
    console.log("  node minutes-diff-sum-all.js --from 2026 --times \"11:00-13:05\" --mode pct --limit 30");
    process.exit(1);
  }
  ...
  console.log(`\n全量可转债分时差值计算`);
  console.log(`参数: from=${opts.from}, to=${opts.to}, times=${opts.timeStart}-${opts.timeEnd}, mode=${mode}, limit=${limit || "全部"}`);
