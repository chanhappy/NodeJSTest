...
export class SM4 {
  constructor(options) {
    ...
    isPkcs7Padding = options.isPkcs7Padding;
  }
  ...
  encrypt(data, key) {
    ...
    if (typeof data === 'string') {
      data = Buffer.from(data, this.inputEncoding || 'utf8')
    ...
    // CBC 分组必须制定 iv
    if (this.mode === CBC && !REG_EXP_KEY.test(this.iv)) {
      throw new TypeError('Invalid value of `iv` option')
    }
    this.iv = this.mode === CBC ? Buffer.from(this.iv, 'hex') : null

    return _encrypt(data, key, this.iv, this.outputEncoding)
  }
  ...
  decrypt(data, key) {
    ...
    // 输入参数校验 `string` | `ArrayBuffer` | `Buffer`
    if (typeof data === 'string') {
      data = Buffer.from(data, this.inputEncoding)
    } else if (data instanceof ArrayBuffer) {
      data = Buffer.from(data)
    }
    if (!Buffer.isBuffer(data)) {
    ...
    return _decrypt(data, key, this.iv, this.outputEncoding)
  }
}
